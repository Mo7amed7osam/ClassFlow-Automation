...
describe('menus by role', () => {
  it('gives the admin users and agents, a coordinator only their groups', () => {
    expect(navFor('admin', 2).map((i) => i.label)).toEqual(['Overview', 'Sessions', 'Recordings', 'Attendance', 'Students', 'Groups', 'Accounts', 'Users', 'Run classes', 'Agents'])
    expect(navFor('admin', 2).find((i) => i.label === 'Users')?.badge).toBe(2)
    expect(navFor('coordinator').map((i) => i.label)).toEqual(['Overview', 'Sessions', 'Recordings', 'Attendance', 'Students', 'My groups', 'Accounts'])
  })
