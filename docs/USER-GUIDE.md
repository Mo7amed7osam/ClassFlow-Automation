...
> The full hands-on guide: installation, permissions, every setting, troubleshooting. For the overview see the [README](../README.md); for the design see [Architecture](ARCHITECTURE.md).
...
- Optional **Automation** (menu → Automation…): DEPI dashboard Run Session, attendance upload and late-joiner correction, Complete Session and Zoom recording links, Google Sheet recording sync, Excel timetable and roster import, Zoom Web meetings for overlapping classes, and automatic co-host. Uses a bundled Node.js + Playwright helper; the core app still runs without it.
