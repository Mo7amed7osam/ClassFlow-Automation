// The Chrome extension's session-worker.js, unchanged in its logic, running inside the app's
// Attendance page. Only the lines that load scripts and re-inject Zoom tabs (Chrome-only) are left out.
// (in the app: name-memory.js and meeting-source.js are loaded by attendance.html)
...
async function startAttendance(message) {
  ...
  const session={id:crypto.randomUUID(),name:(typeof message.name==="string" && message.name.trim()) ? message.name.trim().slice(0,120) : "Meeting "+sequence,sequence,source,active:true,startedAt:new Date().toISOString(),endedAt:null,roster:message.roster || [],observed:{},snapshots:0,admissions:{},waitingAdmissions:{},admissionDiagnostics:{},aiMatches:{},manualMatches:{},rejectedMatches:{}};
