...
| `user_schedules` (migration `0011_user_schedules`) | `user_id` uuid PK → users, `schedules` jsonb (the app's own shape, never read by the server), `count`, `device_name`, `updated_at` |
| `zoom_accounts` (migration `0010_zoom_accounts`) | `id` uuid PK, `user_id` → users, `account_id`, `label`, `zoom_email`, `group_name`, `default_meeting_url`, `preferred_engine`, `password_encrypted` (AES-GCM, migration `0013_zoom_passwords`), `active`, `created_at`, `updated_at`; unique (user_id, lower(account_id)) |
