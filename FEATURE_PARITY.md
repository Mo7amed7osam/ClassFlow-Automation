...
| 2026-09-20 | **An LMS stage ran the whole way against the real DEPI LMS, from Linux.** `lms.run_session` with `dryRun`: signed in, reached `/group_admin`, read the session list for the day, opened the session page for `CAI5_AIS4_S7`, and stopped - "the session page is open and offers Run Session. Nothing was pressed." `lms.complete` refused correctly, reading the page and reporting it "has no Complete Session action; it reads pending", because the session had not been run. `lms.attendance` refused before opening a browser at all, because nothing had collected any names. `PASSED` in dry-run form; a stage that actually presses has still not been run |
| 2026-09-20 | Two workers ran at once against one backend and it shared the queue between them: six jobs succeeded across both, each driving its own Chromium. Duplicate protection holds - the same `Idempotency-Key` twice answered `200 {"duplicate": true}` with the first job's id, not a second execution |
| 2026-09-20 | Browser profiles were landing in `/app`, the container's own writable layer, so every deployment would have signed every account out again and lost the admission ledger with them. .NET resolves `LocalApplicationData` from `XDG_DATA_HOME` on Linux and the image never set it. It does now, at the state volume, which moves the profiles, the ledger and the live-meeting files together. Verified by running a class and looking at what was left on the volume |
| 2026-09-20 | **Classes open by themselves.** Nothing turned a class plan into a job: on Windows that was Task Scheduler, and on Linux there was no replacement, so every stage waited for somebody to ask. `central_backend/scheduling.py` runs beside the sweeper and creates the stages a class is due. Three rules, each tested against a real database: a stage is created **once ever** (the idempotency key is the plan and the stage, so restarts and racing passes reach one job); a class whose time has passed by more than fifteen minutes is **missed and logged, not opened**, so an outage does not fill the queue with the morning's classes at four in the afternoon; and only a coordinator who is turned on has classes opened. Times are read in `Africa/Cairo` and compared in UTC, with the summer and winter offsets tested |
| 2026-09-20 | `tzdata` is a dependency now. `zoneinfo` has no zone database on Windows, so `ZoneInfo("Africa/Cairo")` raised during import and the whole app would not start there. The container has the OS package; this makes the two the same |
| 2026-09-20 | **A meeting is one job, not four.** `class.open` / `class.admit` / `class.attendance` / `class.end` were a guess made before reading the engine; `WebAutoAdmitEngine` is `StartAsync` then `MonitorAsync` then `StopAsync`, which is one browser held for the length of a class. Four jobs would have meant a browser alive between them, and a worker restart orphaning it with nobody able to say whether the class was still running. They are now **`class.run`** - opens the meeting as the host, admits while it is live, closes it - and `class.end` for ending one early from the dashboard |
| 2026-09-20 | `class.run` is built in the worker, on the same `WebAutoAdmitEngine` the Windows app drives. A meeting is opened by a **named** Zoom account or not at all, in that account's own browser profile, so two coordinators' meetings never share a session. A class is held for `durationMinutes` (180 by default): a worker that lost the backend must not sit in an empty meeting for ever holding its only slot. A captcha or one-time code answers `needsVerification` and is not retried, because nobody is watching a server's browser. **Never run against a real Zoom meeting** |
| 2026-09-20 | The scheduler opens the meeting a quarter of an hour before the class and starts the LMS session on the hour - what the Windows app did, and what "Opens 18:45" on the class card means for a 19:00 class. A class still waiting for its Zoom link has no meeting scheduled and is said about; its LMS half still runs, because that needs no link. The scheduler puts its own payloads through the same validator an API caller goes through, so it cannot make one the rest of the system would refuse |
| 2026-09-20 | The worker asks the server for a **Zoom** password the same way it asks for an LMS one, through one shared body rather than two copies that would eventually stop agreeing about what a refusal means. An account with no password saved answers 409 and says so: a coordinator's own PC can sign in by hand and a server cannot. A test caught a real leak on the way - the dry-run path returned before the resolver was taken down, leaving one class's password readable while the next ran |
